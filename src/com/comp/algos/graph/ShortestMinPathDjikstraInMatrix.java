...
public class ShortestMinPathDjikstraInMatrix {
    ...
    dis[0][0] = arr[0][0];
		...
		PriorityQueue<QNode> pq = new PriorityQueue<>(new Comparator<QNode>() {
        
        public int compare(QNode q1, QNode q2){
            if(q1.wt == q2.wt){
                if(q1.x == q2.x){
                    return q1.y>q2.y?1:-1 ;
                }
                return q1.x > q2.x?1:-1;
            }
            return q1.wt > q2.wt?1:-1;
        }
    });
    int[] xc = {1,0,-1,0};
    int[] yc = {0,1,0,-1};
    pq.add(new QNode(0,0,dis[0][0]));
    while(!pq.isEmpty()){
        QNode q = pq.poll();
        for(int i=0; i<4; i++){
            int X = q.x + xc[i];
            int Y = q.y + yc[i];
            if(isInsideGrid(X,Y)){
					if (dis[q.x][q.y] != Integer.MAX_VALUE && dis[q.x][q.y] + arr[X][Y] < dis[X][Y]) {
                    pq.remove(new QNode(X, Y, dis[X][Y]));
                    dis[X][Y] =  dis[q.x][q.y] + arr[X][Y] ;
                    pq.add(new QNode(X, Y, dis[X][Y]));
                }    
