...
public class SubsetMex {
	...
	public static void main(String[] args) throws Exception {
		...
		while (t-- > 0) {
			int n = s.nextInt();
			long[] arr = hp.getLongArray(n, s);
			Arrays.sort(arr);
			long prod1 = arr[n - 1] * arr[n - 2] * arr[n - 3] * arr[n - 4] * arr[n - 5];
			long prod2 = arr[0] * arr[1] * arr[2] * arr[3] * arr[n - 1];
			long prod3 = arr[0] * arr[1] * arr[n - 2] * arr[n - 3] * arr[n - 1];
			//			long prod = Math.max(arr[0] * arr[1], arr[n - 1] * arr[n - 2])
			//			        * Math.max(arr[2] * arr[3], arr[n - 3] * arr[n - 4]) * Math.max(arr[4], arr[n - 5]);
			System.out.println(Math.max(prod2, Math.max(prod1, prod3)));
		}
	}
