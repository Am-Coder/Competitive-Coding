...
public class InfinitSortedArray {
	...
	public static void main(String[] args) throws Exception {
		...
		int a = BinSearchInfinitSortedArr(new int[] { 1, 2, 30, 40, 50, 60, 70, 80, 90 }, 30);
		System.out.println(a);
	}
	
	static int BinSearchInfinitSortedArr(int[] arr, int x) {
		int st = 0;
		int end = 1;
		int ind = -1;
		while (x > arr[end]) {
		...
		ind = binsearch(arr, st, end, x);
		
		return ind;
	}
	
	public static int binsearch(int[] arr, int st, int end, int x) {
		if (st <= end) {
			int mid = st + (end - st) / 2;
			if (x > arr[mid]) {
				return binsearch(arr, mid + 1, end, x);
			} else if (x < arr[mid]) {
				return binsearch(arr, st, mid - 1, x);
			} else {
				return mid;
			}
		}
