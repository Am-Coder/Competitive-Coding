...
public class RPGProtagonist {
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			long p = s.nextLong();
			long f = s.nextLong();
			long cnts = s.nextLong();
			long cnta = s.nextLong();
			long wts = s.nextLong();
			long wta = s.nextLong();
			long ans = solve(cnts, cnta, p, f, wta, wts);
			System.out.println(ans);
		}
	}
	
	public static long solve(long cnts, long cnta, long p, long f, long wta, long wts) {
		long sol = 0;
		for (int i = 0; i <= cnts; i++) {
			...
			if (tp < 0)
				continue;
			tsol += Math.min(tp / wta, t2);
			tsol += i;
			t2 -= Math.min(tp / wta, t2);
			
			if (wts < wta) {
				tsol += Math.min(tf / wts, t1);
				tf -= Math.min(tf / wts, t1) * wts;
				tsol += Math.min(tf / wta, t2);
			} else {
				tsol += Math.min(tf / wta, t2);
				tf -= Math.min(tf / wta, t2) * wta;
				tsol += Math.min(tf / wts, t1);
			}
			sol = Math.max(sol, tsol);
		}
		return sol;
	}
