...
public class SpanningTreeWithMaxDegree {
	...
	public static void main(String[] args) throws IOException {
		...
		vis[maxv] = true;
		for (int chi : adj[maxv]) {
			sol.add(new Pair(maxv, chi));
			vis[chi] = true;
		}
		...
		for (Pair pa : sol) {
			p.println((pa.first + 1) + " " + (pa.second + 1));
		}
		
		p.close();
	}
	
	public static ArrayList<Pair> sol;
	
	public static void dfs(LinkedList<Integer>[] adj, boolean[] vis, int v) {
		vis[v] = true;
		for (int chi : adj[v]) {
			if (!vis[chi]) {
				sol.add(new Pair(v, chi));
				dfs(adj, vis, chi);
			}
		}
	}
