...
public class UncleBogdanAndHappiness {
	...
	public static Pair dfs(int st, int pa) {
		if (!ans) {
			return new Pair(-1, -1);
		}
		if (adj[st].size() == 1 && pa == adj[st].get(0)) {
			if ((hi[st] + pi[st]) % 2 == 0 && Math.abs(hi[st]) <= pi[st]) {
				return new Pair((pi[st] + hi[st]) / 2, pi[st]);
		...
		tot += extra;
		int g1 = 0;
		if ((hi[st] + tot) % 2 == 0 && Math.abs(hi[st]) <= tot) {
			g1 += (tot + hi[st]) / 2;
			if (g1 < good) {
				ans = false;
			} else {
