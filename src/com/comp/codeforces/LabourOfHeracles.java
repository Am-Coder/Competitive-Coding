...
public class LabourOfHeracles {
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			...
			ArrayList<Node> notchild = new ArrayList<>();
			for (int i = 1; i <= n; i++) {
				if (adj[i].size() > 1) {
					notchild.add(arr[i]);
				}
			...
			Collections.sort(notchild, new Comparator<Node>() {
			...
			sol.add(sum);
			for (int i = notchild.size() - 1; i >= 0; i--) {
				Node nn = notchild.get(i);
				int si = adj[nn.val].size() - 1;
				long nsum = sum;
				while(si >0) {
					nsum += (long) nn.wt;
					sol.add(nsum);
