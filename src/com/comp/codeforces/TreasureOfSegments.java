...
public class TreasureOfSegments {
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			...
			for (int i = 0; i < n; i++) {
				int ss = seg[i].first;
				int ee = seg[i].second;
				int ll = lowerbou(end, ss);
				
				int rr = Math.max(0, n - upperbou(st, ee));
				cou = Math.min(cou, ll + rr);
			}
			p.println(cou);
		}
		p.close();
	...
	public static int lowerbou(Integer[] arr, int val) {
		int st = 0;
	...
	public static int upperbou(Integer[] arr, int val) {
		int st = 0;
		int end = arr.length - 1;
		int ans = arr.length;
		while (st <= end) {
