...
public class RescueNibel {
	...
	public static void main(String[] args) throws Exception {
		...
		int n = s.nextInt();
		int k = s.nextInt();
		Helper hp = new Helper(998244353, 1000000);
		
		ArrayList<Pair> inter = new ArrayList<>();
		for (int i = 0; i < n; i++) {
			inter.add(new Pair(s.nextInt(), 1));
			inter.add(new Pair(s.nextInt(), 0));
		}
		Collections.sort(inter);
		long ans = 0;
		int on = 0;
		for (int i = 0; i < inter.size(); i++) {
			if (inter.get(i).second == 1) {
				if (on >= k - 1)
					ans = ((ans % 998244353) + (hp.ncr(on, k - 1) % 998244353)) % 998244353;
				on++;
			} else {
				on--;
			}
