...
public class MaximumDistributedTree {
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			...
			arr.set(n - 2, prod % MOD);
			long ans = 0;
			for (int i = n - 2; i >= 0; i--) {
				ans = (ans + cou.get(i) * arr.get(i)) % MOD;
			}
			p.println(ans % MOD);
		}
	...
	static ArrayList<Long> cou;
	
	public static int dfs(int st, int pa, int n) {
		si[st] = 1;
		for (int chi : adj[st]) {
			if (chi != pa) {
				si[st] += dfs(chi, st, n);
				cou.add(((long) si[chi]) * (long) (n - si[chi]));
			}
		}
