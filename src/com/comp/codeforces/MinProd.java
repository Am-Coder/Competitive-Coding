...
public class MinProd {
	...
	public static long giveDiff(long a, long b, long n) {
		if (b >= n) {
			a = a - n;
		} else {
			a = a - b;
	...
	public static void main(String[] args) throws IOException {
		...
		while (T-- > 0) {
			...
			b1 = b2 = b;
			long q = n;
			a1 = giveDiff(a, left, n);
			n = n - left;
			if (n > 0) {
				b1 = giveDiff(b, right, n);
			}
			n = q;
			b2 = giveDiff(b, right, n);
			n = n - right;
			if (n > 0) {
				a2 = giveDiff(a, left, n);
				
			}
			
			p.println(Math.min(a1 * b1, a2 * b2));
		}
		p.close();
	}
