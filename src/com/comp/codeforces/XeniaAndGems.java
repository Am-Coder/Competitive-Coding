...
public class XeniaAndGems {
	...
	public static void solve(long[] a, long[] b, long[] c) {
		for (long x : a) {
			int ind1 = Arrays.binarySearch(b, x);
			if (ind1 < 0) {
				ind1 = -ind1 - 1;
			}
			if (ind1 == b.length || (ind1 > 0 && b[ind1] > x))
				ind1--;
			
			int ind2 = Arrays.binarySearch(c, x);
			if (ind2 < 0) {
				ind2 = -ind2 - 1;
			}
			if (ind2 < c.length - 1 && c[ind2] < x)
				ind2++;
			if (ind2 == c.length)
				ind2--;
			sol = Math.min(sol, calc(x, b[ind1], c[ind2]));
		}
	}
