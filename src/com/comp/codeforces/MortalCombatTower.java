...
public class MortalCombatTower {
	...
	public static int mortal(int[] arr, int i, int chance, int n) {
		if (i >= n)
			return dp[i][chance] = 0;
		
		if (dp[i][chance] != -1)
			return dp[i][chance];
		
		if (chance == 0) {
			int val1 = mortal(arr, i + 1, 1 - chance, n);
			int val2 = 0;
			if (arr[i] == 1) {
				val1++;
				val2++;
			}
			val2 += mortal(arr, i + 2, 1 - chance, n);
			if (i + 1 < n && arr[i + 1] == 1) {
				val2++;
			}
			return dp[i][chance] = Math.min(val1, val2);
		} else {
			return dp[i][chance] = Math.min(mortal(arr, i + 1, 1 - chance, n), mortal(arr, i + 2, 1 - chance, n));
		}
	}
