...
public class LinkCutCentroids {
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			...
			dfs(1, 0);
			if (c2 == 0) {
				p.println(1 + " " + adj[1].get(0));
				p.println(1 + " " + adj[1].get(0));
			} else {
				pa = null;
				dfs1(c1, c2);
				p.println(pa.first + " " + pa.second);
				p.println(c2 + " " + pa.first);
			}
		}
		p.close();
	}
	
	static int sol, c1, c2;
	...
	static void dfs(int x, int p) {
		...
		for (int chi : adj[x]) {
			...
			dfs(chi, x);
			size[x] += size[chi];
			conmx = Math.max(conmx, size[chi]);
		}
		conmx = Math.max(conmx, n - size[x]);
		if (conmx < sol) {
			sol = conmx;
			c1 = x;
			c2 = 0;
