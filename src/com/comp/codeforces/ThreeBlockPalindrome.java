...
public class ThreeBlockPalindrome {
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			...
			int[][] call = new int[n + 1][27];
			int[][] calr = new int[n + 2][27];
			for (int i = 1; i <= n; i++) {
				for (int j = 1; j <= 26; j++) {
					call[i][j] += call[i - 1][j];
				}
			...
			for (int i = 1; i <= n; i++) {
				for (int j = i; j <= n; j++) {
					...
					int s2 = 0;
					for (int k = 1; k <= 26; k++) {
						s1 = Math.max(s1, Math.min(call[i - 1][k], calr[j + 1][k]));
						s2 = Math.max(s2, call[j][k] - call[i - 1][k]);
					}
					sol = Math.max(sol, 2 * s1 + s2);
				}
			}
			p.println(sol);
		}
