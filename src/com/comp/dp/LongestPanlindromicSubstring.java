...
public class LongestPanlindromicSubstring {
	...
	static void lps(char[] arr, int n) {
		
		int[][] dp = new int[n + 1][n + 1];
		for (int i = 1; i <= n; i++) {
			dp[i][i] = 1;
		}
		int mlen = 1, st = 0, en = 0;
		
		// All substring of length len starting from 2
		for (int len = 2; len <= n; len++) {
			
			for (int j = 1; j <= n - len + 1; j++) {
				int k = j + len - 1;
				
				if (arr[j - 1] == arr[k - 1] && len == 2) {
					dp[j][k] = 2;
					if (mlen < len) {
						mlen = len;
						st = j - 1;
				...
				} else if (arr[j - 1] == arr[k - 1] && dp[j + 1][k - 1] > 0) {
					dp[j][k] = dp[j + 1][k - 1] + 2;
					if (mlen < len) {
						mlen = len;
						st = j - 1;
